...
//' @export
// [[Rcpp::export]]
List cox_reg_sparse_parallel_old(List covrowlist_in,
...
//  double old_lik_correction = 0.0;
  ...
  int * frailty_group_events = new int [maxid];
  double * outcomes_wt = new double [maxid];
  double * frailty_old = new double [maxid];
  double * frailty_old2 = new double [maxid];
  ...
  double * var_newlk = new double [nvar + maxid];
  double * delta_newlk = new double [nvar + maxid];
  double * step = new double [nvar + maxid];
  for (int ivar = 0; ivar < nvar + maxid; ivar++) {
    var_newlk[ivar] = 0.0;
    delta_newlk[ivar] = 0.0;
    step[ivar] = 1.0;
  }
  
  double * gdiagvar = new double [nvar];
  for (int ivar = 0; ivar < nvar; ivar++) gdiagvar[ivar] = 0.0;
  
  double * oldbeta = new double [nvar];
  double * oldbeta2 = new double [nvar];
 ...
 // int Outcome_ntimes = OutcomeTotalTimes.size();
  ...
  // int maxevents = 0;
  // for(int r = Outcome_ntimes -1 ; r >=0 ; r--) maxevents = maxevents < OutcomeTotals[r] ? OutcomeTotals[r] : maxevents;
  // 
  // int * eventsID = new int [Outcome_ntimes*maxevents];
  // 
  // for(int r = 0; r < Outcome_ntimes; r++)
  // {
  //   for(int k = 0; k < maxevents; k++)
  //   {
  //     eventsID[r*maxevents + k] = -1;
  //   }
  // }
  
  // Weight of events summed per person
  for(int rowobs = 0; rowobs < maxobs; rowobs++) {
    int rowid = id[rowobs] - 1;
    if(Outcomes[rowobs] > 0) { 
      outcomes_wt[rowid] += weights[rowobs];
      if (outcomes_wt[rowid] > 0) frailty_group_events[rowid] += Outcomes[rowobs];
    }
  }
...
#pragma omp parallel  default(none) reduction(+:gdiag_private) shared(rowN, maxobs, covrows, coval, beta_local, weights, frailty, Outcomes, obs, id, zbeta) //reduction(+:zbeta[:maxobs])
{
...
#pragma omp parallel  default(none) reduction(+:newlk_private)  shared(efron_wt,wt_average,denom,timein, timeout, zbeta, weights, frailty, Outcomes ,id, ntimes,maxobs)
{
  ...
  for (int rowobs = (rank * maxobs / size); rowobs < ((rank + 1) * maxobs / size) ; rowobs++)
  {
    ...
    if (Outcomes[rowobs] > 0 )
    {
      ...
      wt_average_private[time_index_exit] += weights[rowobs] * static_cast<double>(Outcomes[rowobs]);
    }
...
for (int iter = 0; iter <= MSTEP_MAX_ITER; iter++)
{
 ...
 //   if (update_theta == 0 && i >= nvar) continue;
    ...
    delta_newlk[i] += d2sum[lastvar];  // Meeds to be difference per iteration, so need old - new delta for denominator not just new denominator 
    d2sum[i] = 0.0;
    if (i < nvar) {
      gdiag = -gdiagvar[i];
    } else {
    /* for frailty groups need estimates per frailty group */
      ...
      // frailty_group_times = new int [rowN];
      // for (int ir = 0; ir < rowN; ir++) frailty_group_times[ir] = 0;
      // frailty_group_weights = new int [rowN];
      // for (int ir = 0; ir < rowN; ir++) frailty_group_weights[ir] = 0;
      // for( int covi = 0; covi < rowN; covi++) 
      // {
      //   if (Outcomes[covrows[covi] - 1] > 0) {
      //     if (covi != 0 && frailty_group_times[covi] != frailty_group_times[covi - 1]) {
      //       frailty_events ++;
      //     }
      //     frailty_group_times[frailty_events] = timeout[covrows[covi] - 1] - 1;
      //     frailty_group_weights[frailty_events] += weights[covrows[covi] - 1] - 1;
      //     frailty_group_events[frailty_events] ++;
      //   }
      // }
      // for (int t = 0; t < frailty_events; t++)
      // {
      //   frailty_group_weights[t] /= frailty_group_events[t];
      // }
      // 
...
#pragma omp parallel default(none) shared(rowN, derivMatrix, covrows, coval, weights, frailty, frailty_mean, Outcomes, ntimes, obs, id, timein, timeout, zbeta,i, nvar)
{
    ...
    for (int covi = (rank * rowN / size); covi < (rank + 1) * rowN / size; covi++)
    {
      ...
      double risk = exp(zbeta[(rowobs)] + frailty[rowid]) * weights[rowobs];
  ...
  for(int r = exittimesN ; r >=0 ; r--)
  {
    ...
    // } else {
    //   mean_wt = frailty_group_weights[r];  
    // }
    // 
    // int eventsum = 0;
    // if(i < nvar )  {
    ...
    // } else {
    //   eventsum = frailty_group_events[r];
    // }
    // 
    
    // int frailtygroupevent = 0;
    // if (i >= nvar) 
    //   for(int covi = 0; covi < rowN; covi++) 
    //     if (time == timeout[covrows[covi] - 1] - 1) 
    //       frailtygroupevent = 1;
    // if (i < nvar || frailtygroupevent == 1)
    // {    
      for (int k = 0; k < eventsum; k++)
      {
        ...
        // if (i < nvar) {
          ...
          delta_newlk[i] -= mean_wt*safelog(d2);
          newlk -= mean_wt*safelog(d2);
  //        if ( i == 1 ) Rcout << wt_average[time]*safelog(d2) <<std::endl;
        //}
  ...
  //    Rcout << " gdiag " << gdiag << " hdiag " << hdiag <<std::endl;
//    }
  } 
  
  /* Check if newlk changes are positive or negative and do halving here */
  double dif = 0; 
// if (i <=1) Rcout << "After acuumulators newlk " << newlk << " loglik " << loglik << " delta_newlk " << delta_newlk[i] << " d2sum" << d2sum[i] << " halving" << ((delta_newlk[i] < 0) && (iter_inner > 0)) <<  std::endl;
  
  // /* TODO use theta and coef history rather than last values to reduce memory */
  // if ( (delta_newlk[i] < 0) && (iter_inner > 0)) {
  // 
  //   halving = 1;
  //   delta_newlk[i] = 0.0;   
  //   
  //   if (i < nvar) {
  //   
  //     double updatebeta = (oldbeta[i] + beta[i]) / 2.0;
  //     dif =  beta[i] - updatebeta;
  //     beta[i] = updatebeta;
  //     
  //     if (lambda !=0) {
  //       newlk -= (beta[i] * beta[i]) / (2.0 * lambda);// Include iteration penalty for this covariate
  //       delta_newlk[i] -= (beta[i] * beta[i]) / (2.0 * lambda);// Include iteration penalty for this covariate
  //     }
  //     
  //   
  //   } else if (i >= nvar && recurrent == 1 && theta != 0) {
  // 
  //  //   int rowid = id[covrows[0] - 1] - 1; // might be the same as i - nvar, but if id not contingous then might not be. 
  //     double updatefrailty = (frailty_old[i - nvar] + frailty[i - nvar]) / 2.0;
  //     frailty_old[i - nvar] = frailty[i - nvar];
  //     frailty[i - nvar] = updatefrailty;
  //     
  //     newlk += (frailty_old[i - nvar] - frailty[i - nvar])*nu; 
  //     delta_newlk[i] += (frailty_old[i - nvar]-frailty[i - nvar]) *nu;
  //     
  //   }
  //   
  // 
...
/* Update */
  ...
  // delta_newlk[i] = 0.0;
  // if (i == 0 ) halving = 0; // reset halving for first covariate
  
  if (i < nvar) { 
    oldbeta2[i] = oldbeta[i];
    ...
    if (lambda !=0) {
      ...
      delta_newlk[i] += (beta[i] * beta[i]) / (2.0 * lambda);
    }
 ...
 //   Rcout << " old beta2[i] " << oldbeta2[i] << " old beta[i] " << oldbeta[i] <<" beta[i] "<<beta[i] << std::endl;
    ...
    frailty_old2[i-nvar] = frailty_old[i-nvar];
    frailty_old[i-nvar] = frailty[i-nvar];
     ...
     if ( (theta == 0) || (std::isinf(gdiag)) || (std::isnan(gdiag)) || (std::isinf(hdiag)) || (std::isnan(hdiag)) ) {
       ...
       dif = (gdiag + ((exp(frailty[i - nvar] - frailty_mean) - 1.0) * nu )) /    //-g(d,w)' here is +nu(1-exp(frailty), but gdiag here is negative so use negative form of penalty
         (hdiag + (exp(frailty[i - nvar] - frailty_mean) * nu ) );  // /*- frailty_mean*/again  -g(d,w)' here is -nu(exp(frailty)), but hdiag here is negative so use negative form of penalty
...
//     Rcout << " i " << i - nvar << " old frail2[i - nvar] " << frailty_old2[i - nvar] << " old frail[i - nvar] " << frailty_old[i - nvar] <<" frail[i - nvar] "<<frailty[i - nvar] << std::endl;
     ...
     step[i] = ((2.0 * fabs(dif)) > (step[i] / 2.0)) ? 2.0 * fabs(dif) : (step[i] / 2.0);//Genkin, A., Lewis, D. D., & Madigan, D. (2007). Large-Scale Bayesian Logistic Regression for Text Categorization. Technometrics, 49(3), 291–304. doi:10.1198/004017007000000245
     frailty[i - nvar] -= dif;
  //   delta_newlk[i] -= dif*nu;   // -g(d,w) is + frailty*nu    frailty - old_frailty = -diff   frailty = old_frailty -diff 
  //   newlk -= dif*nu; 
...
//#pragma omp parallel  default(none) reduction(+:newlk_private)  shared(rowN, denom,efron_wt,zbeta,covrows, coval, weights, frailty, frailty_old, Outcomes, ntimes, obs, id, timein, timeout,maxobs, dif, i, nvar)///*,  denom, efron_wt, newlk*/)
{
  ...
  for (int covi = (rank * rowN / size); covi < ((rank + 1) * rowN / size) ; covi++)
  {
    ...
    double xbdif = (i < nvar) ?  dif * coval[row] : 0.0; // don't update zbeta when only patient level frailty terms updated!
...
#pragma omp atomic write
    ...
    double riskdiff = (exp(zbeta_updated + frailty[rowid] ) - riskold) * weights[rowobs];
...
#pragma omp atomic
 ...
 // if ( i == 1 )   Rcout << "After zbeta update newlk " << newlk << " loglik " << loglik << " delta_newlk " << delta_newlk[i] << std::endl;
  ...
  // delete[] frailty_group_times;
  // delete[] frailty_group_weights;
  ...
  frailty_penalty = 0.0;
  for (int ir = 0; ir < maxid ; ir ++ ) {
    frailty_penalty += (frailty[ir] - frailty_mean )*nu ;
   // frailty_old[ir] = frailty[ir];
 //   frailty[ir] = frailty[ir] - frailty_mean;
 ...
 // 
 // 
 //  //  newlk += (frailty[rowid] - frailty_old[rowid]) * nu;
 // //   delta_newlk[rowid + nvar] += (frailty[rowid] - frailty_old[rowid]) *nu;
 // 
 //  }

  /* shift accumulated values to new centred values */
//   for (int ivar = 0; ivar < nvar; ivar++) {
// 
//     IntegerVector covrows_in = covrowlist[ivar];
//     RVector<int> covrows(covrows_in);
//     int rowN = covrows.size();
// 
// #pragma omp parallel  default(none) shared(rowN, denom,efron_wt,zbeta,covrows, coval, weights, frailty, frailty_old, Outcomes, ntimes, obs, id, timein, timeout,maxobs, nvar, ivar)///*,  denom, efron_wt, newlk*/)
// {
//   double *denom_private = new double [ntimes];
//   double *efron_wt_private= new double [ntimes];
// 
...
//     int rowid = id[rowobs] - 1;
// 
//     double riskold = exp(zbeta[rowobs] + frailty_old[rowid]);
// 
//     double riskdiff = (exp(zbeta[rowobs] + frailty[rowid]) - riskold) * weights[rowobs];
// 
//     int time_index_entry =  timein[rowobs] - 1;
//     int time_index_exit =  timeout[rowobs] - 1;
// 
...
//       denom_private[(r)] += riskdiff; // need to update exp(xb1 + xb2 + ) + exp(x2b1 + x2b2 +)
// 
//     if (Outcomes[rowobs] > 0 )  efron_wt_private[(time_index_exit)] += riskdiff;
//   }
//   for (int r = ntimes - 1; r >= 0; r--)
//   {
// #pragma omp atomic
...
// }
  ...
  Rcpp::Rcout << " Iter:  " << iter << " iter_inner : " << iter_inner << " Cox likelihood : " << newlk << "   last likelihood :" << loglik << " theta " << theta << "\n";
  /* Check for convergence and previous halving */
  
  if (((done == 1) || (recurrent == 0))  && (iter_inner > 0) && ((halving == 0 && fabs(1.0 - ((newlk  + frailty_penalty) / (loglik + frailty_penalty_old))) <= MAX_EPS) )) break;
   halving = (iter_inner > 0) && (newlk < loglik);
   update_theta = (done == 0) && (recurrent == 1)  && (halving == 0) && (iter > 0) && (fabs(1.0 - ((newlk + frailty_penalty) / (loglik + frailty_penalty_old))) <= inner_EPS);
//   
   Rcout << " halving " << halving << " update_theta " << update_theta << " done " << done <<  " frailty_penalty "<< frailty_penalty << " nu "<< nu << " convergence " << 1.0 - ((newlk + frailty_penalty) / (loglik + frailty_penalty_old)) << "\n";
   loglik = newlk;
// 
// //   /* halving if not fitting */
  if (halving == 1) {
    Rcout << " Halving as not converging\n" ;

    if (theta != 0 ) {
    for (int rowid = 0; rowid < maxid; rowid++) {
      double updatefrailty = (frailty_old2[rowid] + frailty_old[rowid]) / 2.0;
      frailty_old[rowid] = frailty[rowid];
      frailty[rowid] = updatefrailty;
    //  newlk += (frailty[rowid] - frailty_old[rowid] - frailty_mean)*nu;
 //     Rcout << " old frail2[rowid] " << frailty_old2[rowid] << " old frail[rowid] " << frailty_old[rowid] <<" frail[rowid] "<<frailty[rowid] << std::endl;
    ...
    for (int ivar = 0; ivar < nvar; ivar++) {
      double dif = 0;
      double updatebeta = 0;
      updatebeta = (oldbeta2[ivar] + oldbeta[ivar]) / 2.0;
      oldbeta[ivar] = beta[ivar];
      dif = beta[ivar] - updatebeta;
      beta[ivar] = updatebeta;

  //    Rcout << " old beta2[ivar] " << oldbeta2[ivar] << " old beta[ivar] " << oldbeta[ivar] <<" beta[ivar] "<<beta[ivar] << std::endl;

      if (lambda !=0)  {
        newlk += (oldbeta[ivar] * oldbeta[ivar]) / (2.0 * lambda);
        newlk -= (beta[ivar] * beta[ivar]) / (2.0 * lambda);// Include iteration penalty for this covariate
      }
...
#pragma omp parallel  default(none) reduction(+:newlk_private)  shared(rowN, denom,efron_wt,zbeta,covrows, coval, weights, frailty, frailty_old, Outcomes, ntimes, obs, id, timein, timeout,maxobs, dif, nvar, ivar)///*,  denom, efron_wt, newlk*/)
{
      double *denom_private = new double [ntimes];
      double *efron_wt_private= new double [ntimes];
...
// 
  if ( update_theta == 1) {
    iter_inner = 0; // restart Cox beta fitting
   ...
   Rcout << "*** updating theta ***\n";
    ...
    if (theta != 0 && nu != 0) {
      for (int rowid = 0; rowid < maxid; rowid++) {
        ...
        // double temp2 = 0.0;
        //
        // for (int dj = 1; dj <= frailty_group_events[rowid]; dj++) {
        //   temp2 += safelog((nu + frailty_group_events[rowid] - dj) / (nu + frailty_group_events[rowid]));
        // }
        //lik_correction += temp1 + temp2; // std::lgamma(frailty_group_events[rowid] + nu) - std::lgamma(nu) - frailty_group_events[rowid]*safelog(nu + frailty_group_events[rowid]); // loglikelihood correction for frailty
      }
    }
...
//       double last_min_theta_likl = std::numeric_limits<double>::infinity();
 ...
 //  if (theta >= 0.5) theta = theta/2;  //Fudge to stop runaway
  ...
  //    }
  // 
  //    /* update after centring and possible theta update */
  //    for (int ivar = 0; ivar < nvar; ivar++) {
  // 
  //      double dif = 0.0;
  //      if ( update_theta == 1 ) dif = oldbeta[ivar] - beta[ivar];
  // 
  //      double newlk_private = 0.0;
  // 
  //      IntegerVector covrows_in = covrowlist[ivar];
  ...
  //   }
  // 
  //   int size = omp_get_num_threads(); // get total number of processeshttps://github.com/therneau/survival/blob/master/R/coxpenal.fit.R
  //   int rank = omp_get_thread_num(); // get rank of current ( range 0 -> (num_threads - 1) )
  //   for (int covi = (rank * rowN / size); covi < ((rank + 1) * rowN / size) ; covi++)
  //   {
  //     int row = (covrows[(covi)] - 1); // R_xlen_t is signed long, size_t is unsigned. R vectors use signed so use int throughout
  ...
  // }
  // newlk -= newlk_private;
//  delta_newlk[ivar] -= newlk_private;
//     }
//   }
//    /* update end */
 } else {
