...
#pragma omp parallel  default(none) reduction(+:gdiag_private) shared(frailty_group_events,rowN, i, nvar,  covrows, coval,  weights,  Outcomes, obs) //reduction(+:zbeta[:maxobs])
{
    ...
    for (int covi = rank * rowN / size; covi < (rank + 1) * rowN / size ; covi++) // iter over current covariates
    {
      ...
      if (Outcomes[rowobs] > 0 ) {
        gdiag_private += (i < nvar ? coval[row] : 1.0) * weights[rowobs]; // frailty not in derivative of beta
        if ( i >= nvar ) {
#pragma omp atomic          
...
for (int theta_iter = 0; theta_iter < MSTEP_MAX_ITER && done == 0; theta_iter++) 
{
  ...
  for (int ir = 0; ir < maxobs; ir++) zbeta[ir] = 0.0;
  for (int ir = 0; ir < ntimes*4; ir++) derivMatrix[ir] = 0.0;
  for (int ir = 0; ir < maxid; ir++) frailty[ir] = 0.0;
...
#pragma omp parallel  default(none) shared(rowN, i, nvar, maxobs, covrows, coval, beta_local, weights,  Outcomes, obs, zbeta) //reduction(+:zbeta[:maxobs])
{
...
#pragma omp parallel default(none) shared(rowN, derivMatrix, covrows, coval, weights, Outcomes, ntimes, obs, timein, timeout, zbeta,i, nvar)
{
...
#pragma omp atomic
      ...
      for(int r = exittimesN ; r >=0 ; r--)
      {
        ...
        for (int k = 0; k < OutcomeTotals[r]; k++)
        {
          ...
          d2sum[i] += wt_average[time]*safelog(d2); //wt_average[time]* track this sum to remove from newlk at start of next iteration
  ...
  //  if ( i == 1 )   Rcout << "After diff update newlk " << newlk << " loglik " << loglik << " delta_newlk " << delta_newlk[i] << std::endl;
    /* Update cumulative sums dependent on denominator and zbeta so need to accumulate updates then apply them*/
      double newlk_private = 0.0;
#pragma omp parallel  default(none) reduction(+:newlk_private)  shared(rowN, denom,efron_wt,zbeta,covrows, coval, weights, Outcomes, ntimes, obs, timein, timeout, dif, i, nvar)///*,  denom, efron_wt, newlk*/)
{
      double *denom_private = new double [ntimes];
      double *efron_wt_private= new double [ntimes];
