...
void cox_reg_sparse_parallel_TV_slow( List modeldata, 
   ...
   int  ntimes = Rcpp::max(timeout_in);  // Include time zero but not exit (as exposure for exit is recorded at exit - 1)
   ...
   R_xlen_t  nallvar = nvar + nbeta_spl - nvartv; // -1 to avoid double counting tv beta in nvar and nbeta_spl
   ...
   double* step = new double[nallvar + maxid]{1.0};
   double* gdiagbeta = new double[nallvar]();
 ...
 //  Rcout << "pointing to R vectors in list, ";
   ...
   if (beta_in.length() != nallvar) stop("Beta vector in modeldata list needs to be of same length as number of covariates including splines");
...
//   Rcout << "wrapping R vectors from list, ";
    ...
    double max_time_span = 0;
    
#pragma omp parallel for
    for(R_xlen_t  rowobs = 0; rowobs < maxobs; rowobs++) {
      if (timeout[rowobs] - timein[rowobs] > max_time_span) {
#pragma omp critical
        max_time_span = timeout[rowobs] - timein[rowobs];
        }
      }
  ...
  std::vector<double> zbeta_tv_exposure; 
  zbeta_tv_exposure.reserve(max_time_span);

  std::vector<R_xlen_t> covi_spl = (nvartv > 0) ? std::vector<R_xlen_t>(nvartv) : std::vector<R_xlen_t>(); // index of covariate for each spline variable
  std::vector<std::vector<ExposureRecord>> tv_data = (nvartv > 0) ? std::vector<std::vector<ExposureRecord>>(nbeta_spl) : std::vector<std::vector<ExposureRecord>>(); // list of exposure records for each tv variable
  
//  covi_spl.reserve(nallvar);
//  tv_data.reserve(nallvar);
  if (nvartv > 0) {
   int tv_i = 0;
    for (R_xlen_t  ibeta_notv = 0; ibeta_notv < nvar; ibeta_notv++) { /*Find starting row for each spline variable */ 
   if (tvbeta_spl[ibeta_notv] == 1) {
     covi_spl[tv_i]  = covstart[ibeta_notv] - 1;
//     Rcout << " covi " << covi_spl[tv_i] << " i " << tv_i;
     tv_i++;
   }
    }
  }
  
  Rcout << "MSTEP_MAX_ITER : " << MSTEP_MAX_ITER << "; maxobs : " << maxobs <<"; ntimes : " << ntimes << "; maxid :  " << maxid << "; nvar : " << nvar << "\n tv variables : " <<nvartv  << "; betas for tv variables : " << nbeta_spl << "; nallvar : " << nallvar + maxid  << "; Lookback " << lookback<< std::endl;    
  int iter_theta = 0;
...
#pragma omp for
  ...
  {
    int  time_index_exit = timeout[rowobs] ;
    if ( Outcomes[rowobs]*weights[rowobs] > 0 ) 
    {
      OutcomeCounts[time_index_exit - 1] += 1;
      Wts_allEventtimes[time_index_exit - 1] += weights[rowobs];
    }
  }
}
...
for (int  t = 0; t < ntimes ; t++) {// iter over times
...
for(int  r = eventtimesN -1 ; r >=0 ; r--) wt_average[r] = (OutcomeTotals[r]>0 ? wt_totals[r]/static_cast<double>(OutcomeTotals[r]) : 0.0);

i = 0;
R_xlen_t bs = 0;
R_xlen_t lastID = -1;
//R_xlen_t last_tvcov = -1;
int covi_spl_i = 0; // Integer to track when which spline coeffient is being updated when accumulating the exposures
int covi_tv_i = 0; // Integer to track which time varying variable is being updated to select correct exposure
...
for (R_xlen_t  ibeta = 0; ibeta < nallvar; ibeta++) { /* per observation time calculations */
  gdiag_private = 0.0;

  for (int64_t  covi = covstart[i] - 1; covi < covend[i] ; covi++) {// iter over current covariates
  //  Rcout << "i " <<i << " covi_spl_i " << covi_spl_i << " beta " << ibeta << " spline " << bs << " covi " << covi << std::endl;
      
    R_xlen_t  rowobs = obs[covi] - 1 ;
    int  time_index_entry = timein[rowobs] ;
    int  time_index_exit = timeout[rowobs] ;
...
//              " itimeexit_diff " << itimeexit_diff <<" gdiag update "  << record.value * bspl(itimeexit_diff,bs) * weights[rowobs] <<std::endl; 
  ...
  Rcout << "i " <<i << " tvspline? " << tvbeta_spl[i] << " covi_spl_i " << covi_spl_i << " beta " << ibeta << " spline " << bs << " update "<< gdiagbeta[ibeta] << ", "<< std::endl;
  
  if (tvbeta_spl[i] == 1) {
    if(bs < nspl - 1) { // If next iteration is beyond number of splines then reset
      bs ++;
      covi_spl_i++;
      } else {
        bs = 0;
...
//for (R_xlen_t  ibeta = 0; ibeta < nallvar; ibeta++) Rcout << " ," << gdiagbeta[ibeta] ;
...
/* Set up */
newlk = 0.0;
if (lambda !=0) newlk = -(log(sqrt(lambda)) * nallvar);


loglik = 0.0;
frailty_penalty = 0.0;
d2_sum_private = 0.0;
for (R_xlen_t  ivar = 0; ivar < nallvar + maxid; ivar++) step[ivar] = 1.0; 
...
//   for (R_xlen_t  ibeta_notv = 0; ibeta_notv < nvar; ibeta_notv++) { /*Find starting row for each spline variable */ 
//     if (tvbeta_spl[ibeta_notv] == 1) {
//       covi_spl[i]  = covstart[ibeta_notv] - 1;
...
Rcout << " reset " << std::endl;
for (size_t ntv = 0; ntv < tv_data.size(); ntv++) {
      tv_data[ntv].clear();
      }

lastID = 0;
//#pragma omp parallel  default(none) reduction(+:newlk_private, denom[:ntimes], efron_wt[:ntimes]) private(i,covi_spl,idexposure_multi,idtime_index_multi,lastID)  shared(obs,bspl,tvbeta_spl,nbeta_spl, nspl, nallvar,nvartv,timein, timeout,idstart,idend,id,idn, zbeta, weights,  Outcomes , ntimes,maxobs,lookback, covstart, covend, beta, coval)
//#pragma omp for
for (R_xlen_t  rowobs = 0; rowobs < maxobs; rowobs++) { /* unique per patient/time within each beta /covariate measurement */
  
  R_xlen_t currentID = id[rowobs] - 1;
  int time_index_entry = timein[rowobs] ;  // Time starts at zero but no events at this time to calculate sums. Lowest value of -1 but not used to reference into any vectors
  int time_index_exit = timeout[rowobs] ;

  double zbeta_temp = zbeta[rowobs] >22 ? 22 : zbeta[rowobs];
  zbeta_temp = zbeta_temp < -200 ? -200 : zbeta_temp;
  zbeta[rowobs] = zbeta_temp; // This is the covariate beta value at the start of the exposure period
  
  int span = time_index_exit - time_index_entry ;
  zbeta_tv_exposure.assign(span, zbeta_temp);
  
//Rcout << " reset rowobs " << rowobs << " id " << currentID ;
  ...
  //Rcout << " done" << std::endl;
// This is by rowobs - so by each id so doesn't need to track id
    i = 0;
    covi_spl_i = 0;
    covi_tv_i = 0;
    bs = 0;
    for (R_xlen_t  ibeta = 0; ibeta < nallvar; ibeta++) {/*need to adjust each timepoint R for each tv beta in lookback period */ 
     /* Find the time varying covariates indices */
  //   Rcout << " ibeta " << ibeta ;
      if(tvbeta_spl[i] == 1) {

        int64_t covi = covi_spl[covi_tv_i]; //  covi_spl already zero based as above
 ...
 //     Rcout << " Number of obs to review " <<  tv_data[covi_spl_i].size() <<std::endl;
  //    Rcout << " covi " << covi << " covi_spl_i " << covi_spl_i << " covi " << covi_spl[covi_spl_i]<< std::endl;
    ...
    if (Outcomes[rowobs]*weights[rowobs] > 0 ) {
      /*cumumlative sums for event patients at start of final exposure before event */
      newlk_private += zbeta_tv_exposure[time_index_exit - 1 - time_index_entry] * weights[rowobs];  // Note exit - entry will be the entry prior to exit, eg if times 1 to 5 then 5 times but final time is an exit not entry, and 5 - 1 = 4
      efron_wt[time_index_exit - 1] += exp(zbeta_tv_exposure[time_index_exit - 1 - time_index_entry]); // For exit time 10 and entry at zero, highest exposure index will be 9; so 10 - 1 - 0 = 9
      }
    
    /* sum zbeta for this exposure period into the denominator */
    for (int  r = time_index_exit - 1;  r >= time_index_entry ; r--) { // Need to adjust each time point for all betas at all points in lookback - expensive if don't want to keep temporary array for time period
...
//  Rcout << "Check denom " << std::endl;
//  for (int r = 0; r < ntimes; r++ ) Rcout << ", " << denom[r];
...
for (outer_iter = 0; outer_iter < MSTEP_MAX_ITER && done == 0; outer_iter++) 
{
  ...
  for (iter = 0; iter <= MSTEP_MAX_ITER; iter++)
  {
    ...
    bs = 0;
    covi_spl_i = 0;
    for (R_xlen_t  ibeta = 0; ibeta < nallvar; ibeta++) { 
      ...
      lastID = 0;
      if (nvartv > 0) tv_data[covi_spl_i].clear();
//#pragma omp parallel default(none) reduction(+:derivMatrix[:ntimes*4]) private(tv_data) shared(tvbeta_spl, beta, covstart, covend, coval, weights, Outcomes, ntimes, obs, timein, timeout, zbeta,i,ibeta,bs, nallvar, idstart, idend, id,idn, bspl,lookback) //covn, 
{
//#pragma omp for
      for (int64_t  covi = covstart[i] - 1; covi < covend[i]; covi++) { 
        R_xlen_t  rowobs = (obs[covi] - 1) ;
        int  time_index_entry = timein[rowobs] ; // std vectors use unsigned can be negative though for time 0
        int  time_index_exit = timeout[rowobs] ; // std vectors use unsigned
  ...
  //          Rcout << "clearing" <<std::endl;
    ...
    //      Rcout << " r ";
          for (int  r = time_index_exit - 1;  r >= time_index_entry ; r--) { // keep int for calculations of indices then cast
      //      Rcout << ", " << r ;
            derivMatrix[r] += derivFirst;
            derivMatrix[ntimes + r] += derivSecond;
            }
          if (Outcomes[rowobs]*weights[rowobs] > 0) {
            derivMatrix[(2*ntimes) + time_index_exit - 1] += derivFirst ;
       ...
       //         Rcout << ", " << r ;
             ...
             int itime_diff = r - record.time;
                  ...
                  if ( itime_diff >= 0 && itime_diff < lookback) { 
                      ...
                      derivMatrix[ntimes + r] += derivSecond_tv;   // r <- 0 to exit - 1 ; ntimes = exit 
...
//#pragma omp parallel  default(none) reduction(+:newlk_private, denom_private[:ntimes], efron_wt_private[:ntimes]) private(idtime_index,idexposure)  shared(denom,tvbeta_spl,efron_wt,zbeta,covstart, covend, coval, weights, Outcomes, ntimes, obs, timein, timeout, dif, i,ibeta,bs, nallvar, idstart, idend,id, idn,bspl,lookback, beta)///*,  denom, efron_wt, newlk*/) covn, 
{
//#pragma omp for
        if (nvartv > 0)		tv_data[covi_spl_i].clear();
        lastID = 0;
    ...
    int  time_index_exit = timeout[rowobs] ; // std vectors use unsigned
          ...
          if(tvbeta_spl[i] == 0) {
            ...
            for (int  r = time_index_exit - 1;  r >= time_index_entry ; r--)
                ...
                for (int  r = time_index_entry ;  r < time_index_exit ; r++) {
                  ...
                  int itime_diff = r - record.time;
                    ...
                    if (itime_diff >= 0 && itime_diff < lookback) { /*observation before look back period so drop*/
                      ...
                      double temp_zbeta_tv =  beta[ibeta] * cov_diff; // this is the new beta
                      temp_zbeta_tv =  temp_zbeta_tv >22 ? 22 : temp_zbeta_tv;
                      temp_zbeta_tv =  temp_zbeta_tv < -200 ? -200 :  temp_zbeta_tv;
                      
                      double riskdiff = (exp(temp_zbeta_tv) - exp(temp_zbeta_Old_tv)) * weights[rowobs];
                      denom_private[r] += riskdiff; 
    ...
    {
      newlk_private = 0.0;
#pragma omp parallel for default(none)  reduction(+:newlk_private,denom_private[:ntimes],efron_wt_private[:ntimes]) shared(eventtimesN, maxid, gdiagfrail, idn, idstart, idend, newlk, OutcomeTotalTimes,OutcomeTotals, denom, efron_wt, wt_average, theta, frailty, frailty_mean, nu, step, derivMatrix,  coval, weights, Outcomes, ntimes, obs, timein, timeout, zbeta, nallvar, nbeta_spl, nvartv)
      for (R_xlen_t  i = 0; i < maxid ; i++) 
      {
        ...
        { // iter over current covariates
          ...
          for (int  r = time_index_exit - 1;  r >= time_index_entry ; r--) derivMatrix_private[r] += risk; // keep int for calculations of indices then cast
          
          if (Outcomes[rowobs]*weights[rowobs] > 0) derivMatrix_private[ntimes +  time_index_exit - 1] += risk ;
        ...
        if ( (theta == 0) || (std::isinf(gdiag)) || (std::isnan(gdiag)) || (std::isinf(hdiag)) || (std::isnan(hdiag)) )
        {
          ...
          if (fabs(dif) > step[i + nallvar]) {
            dif = (dif > 0.0) ? step[i + nallvar] : -step[i + nallvar];
          }
          
          step[i + nallvar] = ((2.0 * fabs(dif)) > (step[i + nallvar] / 2.0)) ? 2.0 * fabs(dif) : (step[i + nallvar] / 2.0);//Genkin, A., Lewis, D. D., & Madigan, D. (2007). Large-Scale Bayesian Logistic Regression for Text Categorization. Technometrics, 49(3), 291–304. doi:10.1198/004017007000000245
          frailty[i] -= dif;
        }
    ...
    Rcout << "Beta : " ;
    for (R_xlen_t  ibeta = 0; ibeta < nallvar; ibeta ++) Rcout << beta[ibeta] << " ";
    Rcout << '\n';
...
for (R_xlen_t  r =  timesN - 1; r >= 0; r--)
{
  ...
  R_xlen_t  time = OutcomeTotalTimes[r];
